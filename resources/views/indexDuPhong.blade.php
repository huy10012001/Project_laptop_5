...
$(document).ready(function()
{
    ...
    $("#emailTontaiR").keydown(function() {
    
        var x=$(this).val();
        $.ajax({
			method:'get',
      		url:	 " {{ asset('/checkValidate')}}",
      		data:{email:x},
			...
			success:function(data)
           	{
                ...
                if(data.status=="datontai")
                 {  
                      $('#emailTontai').text('email đã có');
                      $('#emailTontaiR').prop('title', 'email đã có');
               ...
               console.log($('#emailTontaiR').prop('title'));
...
<form id="registerA"   method="post" action="/postRegisterCheckOut">
{{ csrf_field() }}
  ...
  <h3 style="text-align: center;">Tạo tài khoản</h3>
									...
									<input id="emailTontaiR" type="email" class="form-control" name="email" required placeholder="email"  title="email không được để trống và đúng mẫu xxx@gmail.com"><br>
